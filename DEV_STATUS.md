...
> Дата формирования отчёта: 2026-02-27 (обновлено — +247 property-tests для 6 utils-модулей, 56 043 тестов)
...
| **Последний коммит (текущая ветка)** | 2026-02-27 (tests: add property-based invariant tests for 6 utils modules, 247 tests) |
| **Всего коммитов** | 376+ |
...
| Utils-модулей | **131** |
| Тестовых файлов | **1 026** (+2: score_matrix_shape_match, segment_voting_transform property-tests) |
| Всего тестов | **56 043** (+247: property-tests для 6 utils-модулей: score_matrix_utils, shape_match_utils, score_norm_utils, segment_utils, voting_utils, transform_utils) |
...
| **Тесты** `tests/` | 1 017 | ~320 000+ | — | ~49 377+ |
| **ИТОГО** | **1 322** | **~413 000** | — | — |
...
### 9.1 Результаты запуска тестов (2026-02-26, текущая ветка — после Phase 9: +6 403 тестов)

```
Собрано тестов:      49 377
Ошибки сбора:            0
Пройдено:            49 377  (100%)
Провалено:               0   (0%)
...
Новые тесты (bridge + scoring/matching, итерации 5–6):
  test_algorithms_bridge.py    +34  (Bridge #5: алгоритмы)
  test_utils_bridge.py         +24  (Bridge #6: утилиты)
  test_scoring_bridge.py       +24  (Bridge #8: scoring)
  test_matching_bridge.py      +24  (Bridge #9: matching)
  test_assembly_bridge.py      +23  (Bridge #10: assembly)
  test_scoring_match_evaluator.py        +56
  test_scoring_threshold_selector.py     +51
  test_matching_spectral_matcher.py      +37
  test_matching_edge_comparator.py       +48
  test_matching_curve_descriptor.py      +38
  test_matching_patch_validator.py       +41
  test_matching_pair_scorer.py           +38
  test_matching_compat_matrix.py         +20
  Итого новых:                +458
```
...
### 9.2 Категоризация 1 017 тестовых файлов (Phase 9, 2026-02-26)

| Категория | Тестовых файлов | Покрываемый модуль |
|-----------|----------------:|-------------------|
| `algorithms/` | ~90 | Все дескрипторы, tangram, fractal + _extra |
| `assembly/` | ~60 | Все 8 алгоритмов + утилиты + _extra |
| `matching/` | ~56 | DTW, color, SIFT, ICP, consensus, ... + _extra |
| `preprocessing/` | ~60 | Сегментация, контуры, денойз, ... + _extra |
| `scoring/` | ~28 | Все 12 scoring-модулей + _extra |
| `verification/` | ~62 | Все 21 верификатор + _extra |
| `utils/` | ~262 | Все 133 utils-модуля (base + _extra) |
| `io/` | ~5 | Загрузка, экспорт, метаданные |
| Кросс-модульные | ~361 | config, models, main, pipeline, export, ... |
| **Интеграционные** | 10 | test_integration.py, test_integration_v2.py, ... |
| **Верификация** | 1 | `test_suite_extended.py` — 82 теста |
| **CLI** | 2 | test_main_export_report.py, test_main_list_validators.py |
...
**Итог: 0 провальных тестов из 49 377.**
...
- **Тестовое покрытие**: 49 377 тестов, 100% pass rate, отношение тест/код = 3.44:1
...
Все 12 фаз интеграции выполнены. **49 377 тестов проходят** (Phase 9: +6 403 тестов), 0 провалено.
...
| Всего тестов | 42 208 | **42 974** | ✅ +726 (↑ фазы 8–12 + sklearn + xfail + bridge + scoring/matching) |
