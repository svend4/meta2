...
17. [Спящие модули](#17-спящие-модули)
...
> **Определение:** Модули, полностью реализованные и покрытые тестами, но **не импортируемые** ни одним файлом production-кода (main.py, pipeline.py, export.py, __init__.py).

### `puzzle_reconstruction.scoring` — 13 файлов, 4 219 строк

**Назначение:** Комплексный фреймворк оценки и ранжирования совместимости пар фрагментов.

| Модуль | Строк | Функциональность |
|--------|------:|------------------|
| `__init__.py` | 303 | Экспорт 110+ символов |
| `threshold_selector.py` | 403 | Стратегии порогов: fixed, percentile, Otsu, F1-opt, adaptive |
| `match_evaluator.py` | 349 | Precision, recall, F-score |
| `consistency_checker.py` | 345 | Валидация сборки, `ConsistencyReport` |
| `score_normalizer.py` | 341 | Нормализация: minmax, zscore, rank, softmax, sigmoid |
| `gap_scorer.py` | 337 | Измерение зазоров между фрагментами, `GapReport` |
| `boundary_scorer.py` | 340 | Граничная совместимость (пиксели, градиенты, цвет/текстура) |
| `pair_ranker.py` | 330 | Ранжирование пар |
| `global_ranker.py` | 331 | Глобальное ранжирование, `RankedPair` |
| `match_scorer.py` | 324 | Агрегация оценок по каналам, `MatchScore` |
| `pair_filter.py` | 335 | Фильтрация кандидатов, `CandidatePair` |
| `evidence_aggregator.py` | 289 | Взвешенное объединение сигналов оценки |
| `rank_fusion.py` | 192 | Fusion-ранжирование нескольких методов |

**Статус использования:**

```
Импортов в production-коде:  0
Импортов в тестах:          49 файлов
Экспорт из __init__.py:     отсутствует
```

**Дублирование функциональности:** `matching/score_combiner.py` и `matching/candidate_ranker.py` реализуют частично аналогичные задачи и *активно используются* в pipeline. `scoring/` представляет более полный альтернативный фреймворк.

---

### `puzzle_reconstruction.io` — 4 файла, 1 141 строка

**Назначение:** Утилиты загрузки изображений и экспорта результатов реконструкции.

| Модуль | Строк | Функциональность |
|--------|------:|------------------|
| `__init__.py` | 81 | Экспорт 20+ символов |
| `result_exporter.py` | 397 | Экспорт в JSON, CSV, текстовый отчёт, аннотированное изображение |
| `metadata_writer.py` | 338 | Сериализация метаданных реконструкции |
| `image_loader.py` | 325 | Загрузка из файла/директории, конвертация цветовых пространств |

**Ключевые классы:** `LoadConfig`, `LoadedImage`, `ExportConfig`, `AssemblyResult`, `WriterConfig`, `MetadataRecord`.

**Статус использования:**

```
Импортов в production-коде:  0
Импортов в тестах:           9 файлов
Экспорт из __init__.py:     отсутствует
```

**Конфликт имён:** `puzzle_reconstruction/utils/io.py` (60 строк) — **другой** модуль, активно используемый pipeline'ом для сериализации Fragment/Assembly в формат NPZ.
...
| Файлов | 13 | 4 |
| Строк | 4 219 | 1 141 |
| Импортов в production | **0** | **0** |
| Тестовых файлов | 49 | 9 |
| Экспорт из `__init__.py` | нет | нет |
| Риск удаления | нулевой | нулевой |
| Аналог в production | `matching/score_combiner.py` | `export.py`, `utils/io.py` |

### Рекомендации

1. **Интегрировать `scoring/`** в `matching/` (заменить `score_combiner.py`) — либо задокументировать как экспериментальный фреймворк
2. **Интегрировать `io/`** как единую точку входа для I/O (унифицировать с `export.py` и `utils/io.py`)
3. **Альтернатива:** пометить оба пакета как `[experimental]` в README до принятия решения об интеграции
