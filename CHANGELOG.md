...
### Добавлено (property-based тесты text_utils)

- **`tests/test_properties_text_utils.py`** — 143 property-based теста для
  `utils/text_utils.py`:

  **`TextConfig`** (8 тестов):
  - Отрицательный `min_word_gap` → ValueError; `line_threshold` вне [0,1] → ValueError;
    `min_line_height` ≤ 0 → ValueError; граничные значения порога допустимы;
    нулевой `min_word_gap` валиден; пользовательская конфигурация корректна

  **`TextBlock`** (15 тестов):
  - `area = w × h`, нулевая ширина/высота → area = 0, area ≥ 0;
    `center = (x + w/2, y + h/2)`; `n_chars` считает непробельные символы;
    пустой текст → n_chars = 0; n_chars ≤ len(text);
    отрицательные w/h → ValueError; confidence вне [-1, [0,1]] → ValueError;
    граничные значения confidence допустимы

  **`clean_ocr_text`** (16 тестов):
  - Пустая строка → ""; строка из пробелов → ""; обычный текст не изменяется;
    идемпотентность (по умолчанию и с различными входами, parametrized ×4);
    `lowercase=True` → всё в нижнем регистре; `strip_punct=True` → нет знаков пунктуации;
    множественные пробелы → один пробел; ведущие/хвостовые пробелы обрезаются;
    управляющие символы удаляются; `\n` сохраняется; `\t` сворачивается в пробел;
    Unicode NFC-нормализация (NFD == NFC); `strip_punct + lowercase` вместе;
    длина результата ≤ длине ввода

  **`estimate_text_density`** (10 тестов):
  - Результат ∈ [0, 1]; нулевое изображение → 0.0; полностью заполненное → 1.0;
    половина заполнена → 0.5; одиночный ненулевой пиксель → 1/N²;
    монотонность (добавление пикселей увеличивает плотность);
    не 2-D → ValueError; независимость от dtype;
    различные формы (parametrized ×4); неотрицательность

  **`find_text_lines`** (13 тестов):
  - Нулевое изображение → пустой список; возвращает список кортежей;
    y_start < y_end для каждой строки; строки не перекрываются;
    высота каждой строки ≥ min_line_height (parametrized по min_line_height ×4);
    y_end ≤ H; y_start ≥ 0; полное изображение → ≥1 строки;
    текстовое изображение → ≥2 строк; не 2-D → ValueError;
    высокий порог → меньше строк; пустое (H=0) → []

  **`segment_words`** (11 тестов):
  - Нулевая строка → []; возвращает список кортежей; x_start < x_end;
    слова не перекрываются; x_end ≤ W; x_start ≥ 0;
    полностью заполненная строка → 1 слово (0, W); два разделённых блока → ≥2 слов;
    не 2-D → ValueError; одиночный столбец → 1 слово; нулевая ширина → []

  **`compute_text_score`** (9 тестов):
  - Результат ∈ [0, 1]; нулевое изображение → 0.0; неотрицательность;
    текстовое изображение > 0; полное изображение > 0; не 2-D → ValueError;
    различные формы (parametrized ×3); оба изображения в допустимом диапазоне;
    два изображения с разным числом строк — оба в диапазоне

  **`compare_text_blocks`** (13 тестов):
  - Результат ∈ [0, 1]; идентичный текст → 1.0; симметрия;
    симметрия для множества пар (parametrized ×4); оба пустых → 1.0;
    один пустой → 0.0; другой пустой → 0.0; одинаковый одиночный символ → 1.0;
    разные одиночные символы → 0.0; похожий > разного;
    более длинный общий префикс → выше оценка;
    разные позиции, одинаковый текст → 1.0; строки только из пробелов → 0.0

  **`align_text_blocks`** (12 тестов):
  - Пустой список → [] (parametrized ×3); один блок → тот же блок (parametrized ×3);
    то же количество блоков (parametrized ×3); все блоки присутствуют (parametrized ×3);
    top-to-bottom сортируется по Y; left-to-right сортируется по X;
    top-to-bottom при равных Y — по X; left-to-right при равных X — по Y;
    неизвестный режим → ValueError; reading-order → все блоки присутствуют;
    уже отсортированный → остаётся отсортированным; обратный порядок → правильно сортируется

  **`batch_clean_text`** (9 тестов):
  - Пустой список → []; та же длина что и ввод; каждый элемент == clean_ocr_text(t);
    `lowercase=True` применяется ко всем; `strip_punct=True` применяется ко всем;
    один элемент корректно обрабатывается; идемпотентность;
    пустые строки остаются пустыми; `\n` сохраняется в пакетном режиме

### Тестирование (итог 2026-02-27, после добавления property-based text_utils)

- Добавлен файл `tests/test_properties_text_utils.py` (+143 теста)
- Итого: **54 702 теста** (100% проходят, 0 провалено)
- Тестовых файлов: **1 020**

---

### Добавлено (property-based тесты нормализации, расстояний и сигналов)

- **`tests/test_properties_normalization_distance_signal.py`** — 167 property-based тестов для
  трёх утилитных модулей:

  **`utils/normalization_utils.py`** (54 теста):
  - `TestL1Normalize` (7 тестов): L1-норма = 1, длина сохранена, нулевой вектор → нули,
    различные длины, положительный ввод → положительный вывод, равномерный вектор,
    dtype float64
  - `TestL2Normalize` (7 тестов): L2-норма = 1, нулевой вектор → нули, единичный вектор
    не меняется, инвариантность к масштабу, отрицательные значения, известный результат
  - `TestMinmaxNormalize` (7 тестов): вывод ∈ [0,1], min → 0, max → 1, константа → нули,
    длина сохранена, два различных значения, порядок сохранён
  - `TestZscoreNormalize` (6 тестов): mean ≈ 0, std ≈ 1, константа → нули, идемпотентность,
    инвариантность к трансляции
  - `TestSoftmax` (8 тестов): сумма = 1, вывод ∈ [0,1], равномерный ввод → равномерный вывод,
    argmax сохранён, инвариантность к трансляции, одиночный элемент
  - `TestClamp` (6 тестов): вывод ∈ [lo,hi], значения в диапазоне не меняются, форма сохранена,
    все ниже lo → lo, все выше hi → hi
  - `TestSymmetrizeMatrix` (5 тестов): результат = результат.T, симметричный ввод не меняется,
    форма сохранена, диагональ сохранена, идемпотентность
  - `TestZeroDiagonal` (4 теста): диагональ нулевая, недиагональные элементы сохранены,
    форма сохранена, уже нулевая диагональ не меняется

  **`utils/distance_utils.py`** (55 тестов):
  - `TestEuclideanDistance` (7 тестов): d(x,x)=0, ≥0, симметрия, известное значение,
    неравенство треугольника, различные размерности, линейность по масштабу
  - `TestCosineMetrics` (9 тестов): sim(x,x)=1, ∈[−1,1], симметрия, ортогональные → 0,
    противоположные → −1, dist = 1−sim, dist(x,x)=0, dist∈[0,2], нулевой вектор → 0
  - `TestManhattanChebyshev` (9 тестов): manhattan=0 для одинаковых, ≥0, симметрия;
    chebyshev=0, ≥0, ≤manhattan, manhattan≥euclidean, известные значения
  - `TestHausdorffChamfer` (8 тестов): hausdorff(x,x)=0, ≥0, симметрия, одна точка;
    chamfer(x,x)=0, ≥0, симметрия, оба положительны для разделённых множеств
  - `TestPairwiseDistances` (8 тестов): форма (N,N), нулевая диагональ,
    симметрия, неотрицательность — для всех 4 метрик
  - `TestNearestNeighborDist` (4 теста): точка в кандидатах → 0, ≥0,
    один кандидат, совпадает с ручным перебором

  **`utils/signal_utils.py`** (58 тестов):
  - `TestSmoothSignal` (5 тестов): длина сохранена для gaussian/moving_avg,
    константный сигнал неизменен, gaussian уменьшает шум, dtype float64
  - `TestNormalizeSignal` (8 тестов): вывод ∈ [out_min,out_max], min=out_min, max=out_max,
    константа → out_min, длина сохранена, произвольный диапазон, порядок сохранён
  - `TestSignalEnergy` (6 тестов): ≥0, нулевой сигнал → 0, квадратичное масштабирование,
    единичный вектор → 1, известное значение, аддитивность для ортогональных
  - `TestComputeAutocorrelation` (5 тестов): длина 2N−1, центр = 1 (нормировано),
    симметрия для вещественного сигнала, неотрицательный центр, периодический сигнал
  - `TestResampleSignal` (6 тестов): точное число точек (8/16/32/100), первая точка
    сохранена, последняя сохранена, константа → константа, та же длина, один элемент
  - `TestFindPeaks` (6 тестов): индексы допустимы, значения > соседних, расстояние ≥ min_d,
    симметрия find_valleys/find_peaks(-s), константа → нет пиков, единственный пик
  - `TestSegmentSignal` (5 тестов): сегменты не перекрываются, покрывают все точки выше порога,
    above/below дополняют друг друга, нулевой сигнал → нет сегментов, единица → 1 сегмент
  - `TestPhaseShift` (5 тестов): идентичные сигналы, сдвиг в допустимом диапазоне,
    peak ≤ 1, длина кросскорреляции, нулевой сигнал → нулевая кросскорреляция

### Тестирование (итог 2026-02-27)

- Добавлен файл `tests/test_properties_normalization_distance_signal.py` (+167 тестов)
- Итого: **54 559 тестов** (100% проходят, 0 провалено)
- Тестовых файлов: **1 019**
