...
| **Общая готовность кода** | ~100% реализован, ~100% подключён через реестры |
| **Тестовое покрытие** | 100% модулей, 99.97% тестов проходят |
| **Исходных модулей** | 305 `.py` файлов |
| **Строк исходного кода** | 93 279 |
| **Тестовых файлов** | 822 |
| **Строк тестового кода** | 267 359 |
| **Всего тестов (pytest)** | 42 219 |
| **Тестов пройдено** | 42 208 (99.97%) |
| **Тестов провалено** | 0 (0%) |
| **Коммитов** | 263 |
| **Алгоритмов сборки в CLI** | 8/8 + auto + all |
| **Матчеров** | 13+ через `matcher_registry` + конфигурируемые веса |
| **Модулей предобработки** | 38 через `PreprocessingChain` |
| **Верификаторов** | 21 через `VerificationSuite` |
...
🌱 КОРНИ    — preprocessing/ (38 модулей)
             ...
             Доступно: 38/38 через PreprocessingChain (конфигурируемая цепочка)
...
| `beam_search.py` | ✅ В CLI | O(W·N²) | Средний пазл (по умолчанию) |
| `gamma_optimizer.py` | ✅ В CLI | O(I·N²) | Крупный пазл, SOTA |
| `genetic.py` | ✅ В CLI | O(G·P·N²) | 15–40 фрагментов |
| `exhaustive.py` | ✅ В CLI | O(N!) | ≤8 фрагментов, гарантированный оптимум |
| `ant_colony.py` | ✅ В CLI | O(I·A·N²) | 20–60 фрагментов |
| `mcts.py` | ✅ В CLI | O(S·D) | 6–25 фрагментов |
| `auto` | ✅ В CLI | — | Автовыбор метода по числу фрагментов |
| `all` | ✅ В CLI | — | Все 8 методов, выбор лучшего по score |
...
**Матчеры через `matcher_registry.py`**

Веса задаются в `MatchingConfig.matcher_weights` (JSON/YAML-конфиг). По умолчанию:

| Модуль | Статус | Вес (default) | Что измеряет |
|---|---|---|---|
...
**Дополнительные матчеры (регистрируются через `@register` декоратор)**
...
**Инфраструктура агрегации (активна через `MatchingConfig`)**
...
### Все провалы устранены (0 failures)

На 25 февраля 2026 г. все 133 ранее провалившихся теста исправлены.
...
```bash
# Одиночный метод
python main.py --input scans/ --output result.png --method beam
python main.py --input scans/ --output result.png --method gamma
python main.py --input scans/ --output result.png --method genetic --genetic-pop 50
python main.py --input scans/ --output result.png --method exhaustive  # N≤8
python main.py --input scans/ --output result.png --method ant_colony --aco-ants 20
python main.py --input scans/ --output result.png --method mcts --mcts-sim 200

# Интеллектуальный выбор и research mode
...
Файл `assembly/parallel.py` содержит все 8 зарегистрированных алгоритмов.
`run_all_methods()`, `run_selected()`, `pick_best()`, `summary_table()` — активны.
...
### Все 38 модулей через `PreprocessingChain`

`preprocessing/chain.py` — конфигурируемая цепочка фильтров. Фильтры задаются в конфиге:

```yaml
preprocessing:
  chain: [color_norm, denoise, deskew, contrast, morphology]
```

**Всегда активные модули:**
...
| `color_norm.py` | Нормализация цвета (CLAHE) |

**Дополнительные модули (активируются через конфиг):**
...
### Все 21 модуль через `VerificationSuite`

`verification/suite.py` — конфигурируемый набор верификаторов. Компоненты задаются в `VerificationConfig`.

**Всегда активный:**

| Модуль | Функция |
|---|---|
| `ocr.py` | `verify_full_assembly()` — OCR-связность (Tesseract, опционально) |

**Доступные компоненты (активируются через `VerificationConfig`):**
...
| **2** | Высокий | ✅ Выполнена | Assembly Registry: все 8 алгоритмов в CLI + auto + all |
| **3** | Средний | ✅ Выполнена | Matcher Registry: `matcher_registry.py` + конфигурируемые веса |
| **4** | Средний | ✅ Выполнена | Preprocessing Chain: `chain.py` + конфигурируемые фильтры |
| **5** | Средний | ✅ Выполнена | Verification Suite: `suite.py` + 21 верификатор |
| **6** | Низкий | ✅ Выполнена | Infrastructure Utils: EventBus + ResultCache + MetricTracker |
| **7** | Низкий | ✅ Выполнена | Research Mode: `--research` + `--method all` + consensus + JSON-экспорт |
...
- [x] Конфигурируемые матчеры через config.yaml
- [x] Конфигурируемая цепочка предобработки
- [x] VerificationSuite с отчётом качества A–F
- [x] Все 42 208 тестов проходят (0 failures)
...
Все 133 ранее провалившихся теста устранены (25 февраля 2026).

### Устранённые технические долги

| Было | Стало |
|---|---|
| Жёсткие веса матчеров в `pairwise.py` | ✅ Конфигурируемые через `MatchingConfig.matcher_weights` |
...
1. **`pytest` без установки** — запускать через `python -m pytest`
2. **Нет Docker-образа** — деплой требует ручной установки зависимостей
...
| Алгоритмы сборки | **100% кода**, **100% в CLI** (8/8 + auto + all) |
...
*Документ обновлён 2026-02-25. Актуальная ветка: `claude/puzzle-text-docs-3tcRj`.*
