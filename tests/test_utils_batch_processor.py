"""Tests for puzzle_reconstruction/utils/batch_processor.py"""
import pytest

from puzzle_reconstruction.utils.batch_processor import (
    ProcessConfig,
    ProcessItem,
    BatchSummary,
    make_processor,
    process_items,
    filter_successful,
    retry_failed_items,
    split_batch,
    merge_batch_results,
)


# ─── helpers ──────────────────────────────────────────────────────────────────

def _ok(x):
    return x * 2


def _fail(x):
    raise RuntimeError("intentional error")


def _flaky():
    """Returns a function that fails the first N calls, then succeeds."""
    calls = [0]

    def fn(x):
        calls[0] += 1
        if calls[0] <= 1:
            raise ValueError("flaky")
        return x
    return fn


def _run(items, fn=_ok, **cfg_kwargs) -> BatchSummary:
    cfg = ProcessConfig(**cfg_kwargs) if cfg_kwargs else None
    return process_items(items, fn, cfg)


# ─── TestProcessConfig ────────────────────────────────────────────────────────

class TestProcessConfig:
    def test_defaults(self):
        cfg = ProcessConfig()
        assert cfg.batch_size == 32
        assert cfg.max_retries == 0
        assert cfg.stop_on_error is False
        assert cfg.verbose is False

    def test_batch_size_zero_raises(self):
        with pytest.raises(ValueError):
            ProcessConfig(batch_size=0)

    def test_batch_size_negative_raises(self):
        with pytest.raises(ValueError):
            ProcessConfig(batch_size=-1)

    def test_max_retries_negative_raises(self):
        with pytest.raises(ValueError):
            ProcessConfig(max_retries=-1)

    def test_batch_size_one_ok(self):
        cfg = ProcessConfig(batch_size=1)
        assert cfg.batch_size == 1

    def test_max_retries_zero_ok(self):
        cfg = ProcessConfig(max_retries=0)
        assert cfg.max_retries == 0


# ─── TestProcessItem ──────────────────────────────────────────────────────────

class TestProcessItem:
    def test_construction(self):
        pi = ProcessItem(index=2, success=True, result=42)
        assert pi.index == 2
        assert pi.success is True
        assert pi.result == 42
        assert pi.error is None
        assert pi.retries == 0

    def test_index_negative_raises(self):
        with pytest.raises(ValueError):
            ProcessItem(index=-1, success=True)

    def test_retries_negative_raises(self):
        with pytest.raises(ValueError):
            ProcessItem(index=0, success=True, retries=-1)

    def test_failed_item(self):
        pi = ProcessItem(index=0, success=False, error="oops")
        assert pi.success is False
        assert pi.error == "oops"


# ─── TestBatchSummary ─────────────────────────────────────────────────────────

class TestBatchSummary:
    def _make(self, items, n_success, n_failed, n_retried=0):
        return BatchSummary(
            total=len(items),
            n_success=n_success,
            n_failed=n_failed,
            n_retried=n_retried,
            items=items,
        )

    def test_success_ratio_empty(self):
        s = self._make([], 0, 0)
        assert s.success_ratio == pytest.approx(0.0)

    def test_success_ratio_full(self):
        items = [ProcessItem(i, True) for i in range(5)]
        s = self._make(items, 5, 0)
        assert s.success_ratio == pytest.approx(1.0)

    def test_success_ratio_half(self):
        items = [
            ProcessItem(0, True),
            ProcessItem(1, False, error="e"),
        ]
        s = self._make(items, 1, 1)
        assert s.success_ratio == pytest.approx(0.5)

    def test_failed_indices(self):
        items = [
            ProcessItem(0, True),
            ProcessItem(1, False, error="e"),
            ProcessItem(2, True),
            ProcessItem(3, False, error="e"),
        ]
        s = self._make(items, 2, 2)
        assert s.failed_indices == [1, 3]

    def test_successful_results(self):
        items = [
            ProcessItem(0, True, result="a"),
            ProcessItem(1, False, error="e"),
            ProcessItem(2, True, result="b"),
        ]
        s = self._make(items, 2, 1)
        assert s.successful_results == ["a", "b"]

    def test_negative_count_raises(self):
        with pytest.raises(ValueError):
            BatchSummary(total=-1, n_success=0, n_failed=0,
                         n_retried=0, items=[])


# ─── TestMakeProcessor ────────────────────────────────────────────────────────

class TestMakeProcessor:
    def test_returns_process_config(self):
        cfg = make_processor()
        assert isinstance(cfg, ProcessConfig)

    def test_custom_batch_size(self):
        cfg = make_processor(batch_size=16)
        assert cfg.batch_size == 16

    def test_stop_on_error(self):
        cfg = make_processor(stop_on_error=True)
        assert cfg.stop_on_error is True


# ─── TestProcessItems ─────────────────────────────────────────────────────────

class TestProcessItems:
    def test_empty_input(self):
        s = _run([])
        assert s.total == 0
        assert s.n_success == 0

    def test_all_success(self):
        s = _run([1, 2, 3])
        assert s.n_success == 3
        assert s.n_failed == 0

    def test_results_correct(self):
        s = _run([5, 6])
        assert s.successful_results == [10, 12]

    def test_all_fail(self):
        s = _run([1, 2], fn=_fail)
        assert s.n_failed == 2
        assert s.n_success == 0

    def test_failed_items_have_error(self):
        s = _run([1], fn=_fail)
        assert s.items[0].error is not None
        assert s.items[0].success is False

    def test_indices_correct(self):
        s = _run([10, 20, 30])
        for i, item in enumerate(s.items):
            assert item.index == i

    def test_stop_on_error_stops_early(self):
        s = process_items(
            [1, 2, 3, 4, 5],
            _fail,
            ProcessConfig(stop_on_error=True),
        )
        assert len(s.items) < 5

    def test_stop_on_error_false_processes_all(self):
        s = process_items(
            [1, 2, 3],
            _fail,
            ProcessConfig(stop_on_error=False),
        )
        assert len(s.items) == 3

    def test_retries_on_flaky(self):
        fn = _flaky()
        s = process_items(
            [1],
            fn,
            ProcessConfig(max_retries=2),
        )
        assert s.n_success == 1
        assert s.items[0].retries >= 1

    def test_no_retry_on_success(self):
        s = _run([1, 2, 3])
        for item in s.items:
            assert item.retries == 0

    def test_none_cfg_uses_default(self):
        s = process_items([1, 2], _ok, None)
        assert s.n_success == 2

    def test_total_equals_input_length(self):
        items = list(range(7))
        s = _run(items)
        assert s.total == 7


# ─── TestFilterSuccessful ─────────────────────────────────────────────────────

class TestFilterSuccessful:
    def test_all_success(self):
        s = _run([1, 2, 3])
        assert len(filter_successful(s)) == 3

    def test_some_fail(self):
        s = process_items([1, 2, 3], _fail)
        assert filter_successful(s) == []

    def test_mixed(self):
        results = [
            ProcessItem(0, True, result="a"),
            ProcessItem(1, False, error="e"),
        ]
        summary = BatchSummary(total=2, n_success=1, n_failed=1,
                               n_retried=0, items=results)
        ok = filter_successful(summary)
        assert len(ok) == 1
        assert ok[0].result == "a"


# ─── TestRetryFailedItems ─────────────────────────────────────────────────────

class TestRetryFailedItems:
    def test_retry_with_fixed_fn(self):
        """First attempt fails; retry with working fn."""
        original = [1, 2, 3]
        s = process_items(original, _fail)
        assert s.n_failed == 3
        s2 = retry_failed_items(original, s, _ok)
        assert s2.n_success == 3

    def test_no_failed_items_empty(self):
        original = [1, 2]
        s = _run(original)
        s2 = retry_failed_items(original, s, _ok)
        assert s2.total == 0


# ─── TestSplitBatch ───────────────────────────────────────────────────────────

class TestSplitBatch:
    def test_exact_split(self):
        batches = split_batch([1, 2, 3, 4], batch_size=2)
        assert batches == [[1, 2], [3, 4]]

    def test_remainder(self):
        batches = split_batch([1, 2, 3, 4, 5], batch_size=2)
        assert len(batches) == 3
        assert batches[-1] == [5]

    def test_empty(self):
        batches = split_batch([], batch_size=4)
        assert batches == []

    def test_batch_size_exceeds_list(self):
        batches = split_batch([1, 2], batch_size=10)
        assert batches == [[1, 2]]

    def test_batch_size_one(self):
        batches = split_batch([10, 20, 30], batch_size=1)
        assert batches == [[10], [20], [30]]

    def test_batch_size_zero_raises(self):
        with pytest.raises(ValueError):
            split_batch([1, 2], batch_size=0)


# ─── TestMergeBatchResults ────────────────────────────────────────────────────

class TestMergeBatchResults:
    def test_empty_raises(self):
        with pytest.raises(ValueError):
            merge_batch_results([])

    def test_single_summary(self):
        s = _run([1, 2, 3])
        merged = merge_batch_results([s])
        assert merged.total == 3
        assert merged.n_success == 3

    def test_two_summaries(self):
        s1 = _run([1, 2])
        s2 = _run([3, 4, 5])
        merged = merge_batch_results([s1, s2])
        assert merged.total == 5
        assert merged.n_success == 5

    def test_indices_offset(self):
        s1 = _run([10])
        s2 = _run([20, 30])
        merged = merge_batch_results([s1, s2])
        indices = [item.index for item in merged.items]
        assert indices == [0, 1, 2]

    def test_n_failed_summed(self):
        s1 = _run([1], fn=_fail)
        s2 = _run([2], fn=_fail)
        merged = merge_batch_results([s1, s2])
        assert merged.n_failed == 2
